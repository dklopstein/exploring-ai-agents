...
class Agent:
    ...
    def MC_run(self, num_simulation, tester=False):
        ...
        for simulation in range(num_simulation):
            ...
            trajectory = [self.simulator.state]
            ...
            for i in range(len(trajectory)):
                state = trajectory[i]
                self.S_MC[state] += reward * (DISCOUNT ** (len(trajectory) - i - 1))
                self.N_MC[state] += 1
    ...
    def TD_run(self, num_simulation, tester=False):
        ...
        for simulation in range(num_simulation):
            ...
            while not self.simulator.game_over():
                ...
                self.N_TD[state] += 1
                self.TD_values[state] += \
                    self.alpha(self.N_TD[state]) * (reward + DISCOUNT * self.TD_values[next_state] - self.TD_values[state])
    ...
    def Q_run(self, num_simulation, tester=False, epsilon=0.4):
        ...
        for simulation in range(num_simulation):
            ...
            while not self.simulator.game_over():
                ...
                n_s, reward = self.make_one_transition(a)
                self.N_Q[s][a] += 1
                self.Q_values[s][a] += \
                    self.alpha(self.N_Q[s][a]) * (reward + DISCOUNT * max(self.Q_values[n_s]) - self.Q_values[s][a])
